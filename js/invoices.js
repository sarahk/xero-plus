$(document).ready(function () {
    ...
    if ($('#tInv4Contract').length) {
        var tInv4Contract = $('#tInv4Contract').DataTable({
            "ajax": {
            ...
            buttons: [{
                ...
                action: function (e, dt, node, config) {
                    //dt.ajax.reload();
                    tInvoices.ajax.url('/json.php?endpoint=Invoices&action=Contract&repeating_invoice_id=' + repeating_invoice_id).load();
                }
            }, {
                text: 'Overdue',
                className: 'btn mr-1',
                action: function (e, dt, node, config) {
                    tInvoices.ajax.url('/json.php?endpoint=Invoices&action=Contract&button=overdue&repeating_invoice_id=' + repeating_invoice_id).load();
                }
            }, , {
                text: 'Paid',
                className: 'btn mr-1',
                action: function (e, dt, node, config) {
                    tInvoices.ajax.url('/json.php?endpoint=Invoices&action=Read&button=paid&repeating_invoice_id=' + repeating_invoice_id).load();
                }
            }, {
                text: 'Void',
                className: 'btn mr-1',
                action: function (e, dt, node, config) {
                    tInvoices.ajax.url('/json.php?endpoint=Invoices&action=Read&button=voided&repeating_invoice_id=' + repeating_invoice_id).load();
                }
            }]
