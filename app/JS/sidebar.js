...
const CLASSES = {OPEN: 'sidebar-open'};
const EVENTS = {
...
export class Sidebar {
    ...
    show() {
        ...
        if (!dispatchCancelable(this.el, EVENTS.SHOW)) return;
    ...
    hide() {
        ...
        if (!dispatchCancelable(this.el, EVENTS.HIDE)) return;
        ...
        onTransitionEndOnce(this.el, () => {
            this._transitioning = false;
            dispatch(this.el, EVENTS.HIDDEN);
        });
    }
    ...
    isOpen() {
        return this.el.classList.contains(CLASSES.OPEN);
    }
}
...
function handleAutohideClick(e) {
    const t = e.target;
    const isToggle = t.closest(SELECTORS.TOGGLE);
    const isSidebar = t.closest(SELECTORS.CONTAINER);
...
export const Sidebars = {
    init(root = document) {
        // Data-API
        root.querySelectorAll(SELECTORS.TOGGLE).forEach((btn) => {
